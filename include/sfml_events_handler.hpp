...
public:
    ...
    private:
        ...
        void HandleAutoZoom() {
            ...
            const int px = static_cast<int>((AppState::AUTO_ZOOM_TARGET_X - state_.viewport.x_min) /
                                            state_.viewport.width() * render_settings_.width);

            const int py = static_cast<int>((AppState::AUTO_ZOOM_TARGET_Y - state_.viewport.y_min) /
                                            state_.viewport.height() * render_settings_.height);

            ZoomToPoint(px, py, /*zoom_in=*/true, 0.95);
            state_.need_rerender = true;
        }
